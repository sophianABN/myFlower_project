    ...
    <div v-if="plant" class="bg-white rounded-lg shadow-lg overflow-hidden">
      <div class="md:flex">
        ...
        <div class="md:w-1/2 p-6">
          ...
          <div class="mb-4">
            <h2 class="text-lg font-semibold text-leaf-green">Prochain arrosage</h2>
...
export default {
  ...
  computed: {
    nextWateringDate() {
      if (!this.plant || !this.plant.lastWatered) return null;
      
      const lastWatered = new Date(this.plant.lastWatered);
      const nextWatering = new Date(lastWatered.getTime() + this.plant.waterNeeds.frequency * 24 * 60 * 60 * 1000);
      return nextWatering;
    }
  },
  async fetch() {
